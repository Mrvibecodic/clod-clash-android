...
| **Ядро** | [MetaCubeX/mihomo](https://github.com/MetaCubeX/mihomo), подключено submodule'ом, **не форкается**; точечные патчи лежат в `.github/patch-core/` и накладываются на submodule при сборке в CI (для локальной сборки наложить их так же, `patch -p1` из каталога ядра); закреплено на стабильном теге `v1.19.30` (`ac017cdd`) — в `.gitmodules` стоит `branch = Alpha`, но CI тянет submodule только по `--init`, без `--remote`, поэтому пин держится. Двигать пин осознанно и вместе с обоими `go.mod` | GPL-3.0 |
